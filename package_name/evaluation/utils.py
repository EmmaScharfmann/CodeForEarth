...
from package_name.evaluation.metrics import compute_conditional_probabilities, compute_BSS_clusters_target, compute_inputs_to_calc_quantile_exceedance
...
def compute_BSS_clusters_target_from_pca(
    ...
    if input_climatology is None or targets_categorical_climatology is None:
        ...
        conditional_probs_targets = compute_conditional_probabilities(
            cluster_probs=cluster_probs_training.T, targets_categorical=targets_categorical_climatology)

    return compute_BSS_clusters_target(cluster_probs=cluster_probs_testing.transpose(1, 0),targets_categorical=targets_categorical_test, targets_categorical_climatology=targets_categorical_climatology, conditional_probs=conditional_probs_targets)
...
def compute_quantile_exceedance_BSS_from_pca(
    ...
    cluster_probs, targets_categorical, targets_categorical_climatology, conditional_probs_targets = compute_inputs_to_calc_quantile_exceedance(
    ...
    return compute_BSS_clusters_target(cluster_probs=cluster_probs, targets_categorical=targets_categorical, targets_categorical_climatology=targets_categorical_climatology, conditional_probs=conditional_probs_targets)
...
def compute_BSS_clusters_target_from_vae(
    ...
    if input_climatology is None or targets_categorical_climatology is None:
        ...
        conditional_probs_targets = compute_conditional_probabilities(
        cluster_probs=cluster_probs_training, targets_categorical=targets_categorical_climatology)

    return compute_BSS_clusters_target(cluster_probs=cluster_probs_testing, targets_categorical=targets_categorical_test, targets_categorical_climatology=targets_categorical_climatology, conditional_probs=conditional_probs_targets)
...
def predict_clusters(
    ...
    if inputs.ndim == 4:
        ...
        ensemble_input = inputs.reshape(
            n_times * n_members, n_latitudes, n_longitudes
        )
        cluster_probabilities = vae.encode(
            input=flatten_input(X=ensemble_input)
        )["clusters_pred"]
